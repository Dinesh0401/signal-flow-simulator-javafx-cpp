 ...
 *
 * Styled to match MATLAB Simulink: white background with a light gray grid.
 */
public class WorkspaceCanvas extends Pane {
    ...
    private static final Color GRID_COLOR = Color.web("#DCDCDC");
    ...
    // --- Grid drawing (Simulink-style light gray line grid) ---
    ...
    private void drawGrid() {
        ...
        // Vertical grid lines
        for (double x = GRID_SPACING; x < PREFERRED_WIDTH; x += GRID_SPACING) {
            gc.strokeLine(x, 0, x, PREFERRED_HEIGHT);
        }

        // Horizontal grid lines
        for (double y = GRID_SPACING; y < PREFERRED_HEIGHT; y += GRID_SPACING) {
            gc.strokeLine(0, y, PREFERRED_WIDTH, y);
        }
    }
