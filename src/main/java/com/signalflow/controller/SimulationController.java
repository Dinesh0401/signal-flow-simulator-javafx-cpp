...
public class SimulationController {
    ...
    /** Configurable simulation step size (seconds). When positive, the
     *  per-frame dt is clamped to this value for deterministic stepping. */
    ...
    /**
     * Sets the simulation step size. When positive, the per-frame dt
     * is clamped to this value for more deterministic simulation.
     * Set to 0 (or negative) to use the real-time frame delta.
     */
    ...
    private void onFrame(long now) {
        ...
        // If a step size has been configured, use it instead of the frame delta
        if (stepSize > 0) {
            dt = stepSize;
        }
